...
public class BookingController {
	...
	public @ResponseBody Object findCylinderBookedWithDealerFY(@RequestParam Integer year,
			@RequestParam Integer bookToID) {

		logger.info("Getting User Details..");
		AppResponse appResponse = new AppResponse();
		try {
			int bookingCount = bookingService.findCylinderBookedWithDealerFY(year, bookToID);
			appResponse.setStatus("OK");
	...
	public @ResponseBody Object findCylinderBookedCustomerFY(@RequestParam Integer year,
			@RequestParam Integer bookToID) {
		logger.info("Getting User Details..");
		AppResponse appResponse = new AppResponse();
		try {
			Map<String, Integer> bookingCount = bookingService.cylinderBookBYeachCustForFY(year, bookToID);
			return bookingCount;
		} catch (Exception e) {
			logger.error("Error While Getting User Details.." + e);
			e.printStackTrace();
	...
	public @ResponseBody Object findCylinderSoldProductWiseFY(@RequestParam Integer year,
			@RequestParam Integer connectionType, @RequestParam Integer bookToID) {
		logger.info("Getting Sold Cylinder Details..");
		AppResponse appResponse = new AppResponse();
		try {
			Map<String, Integer> bookingReport = bookingService.findCylinderSoldProductWiseFY(year, connectionType,
					bookToID);
	...
	@RequestMapping(value = Constant.GET_SALES_REPORT, method = RequestMethod.GET)
	public @ResponseBody Object salesReportForDealer(@RequestParam Integer bookToId, @RequestParam Integer year) {
		logger.info("Getting Sold Cylinder Details..");
		AppResponse appResponse = new AppResponse();
		try {
			List<Reports> reportMap = bookingService.salesReportForDealer(bookToId, year);
			return reportMap;
		} catch (Exception e) {
			logger.error("Error While Getting Booking Details.." + e);
			e.printStackTrace();
